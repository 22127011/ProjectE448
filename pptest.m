...
for i=1:length(nll(:,1))
    if df(i) >= distance_to_travel 
         distance_to_travel = distance_to_travel + distance_per_segment(step);
         plot(nll(track:i,2),nll(track:i,1));
         track = i;
         step = step + 1; 
    end
end
plot(nll(track:length(nll(:,1)),2),nll(track:length(nll(:,1)),1));
axis('equal');
title('Segmented route', 'FontSize', 16);
xlabel('Longitude (°)', 'FontSize', 12);
ylabel('Latitude (°)', 'FontSize', 12);
text(nll(1,2),nll(1,1),'START',FontSize=10);
text(nll(953,2),nll(953,1),'END',FontSize=10);
hold off;
...
for i=1:length(nll(:,1))
    if df(i) >= distance_to_travel 
         distance_to_travel = distance_to_travel + distance_per_segment(step);
         plot(df(track:i),alt(track:i));
         track = i;
         step = step + 1; 
    end
end
plot(df(track:length(nll(:,1))),alt(track:length(nll(:,1))));
title('Elevation vs Distance', 'FontSize', 16);
...
for i=1:x+1
    if dist(i) >= distance_to_travel 
         distance_to_travel = distance_to_travel + distance_per_segment(step);
         plot(dist(track:i),s(track:i));
         track = i;
         step = step + 1; 
    end
end
plot(dist(track:x+1),s(track:x+1));
% avga = ones(size,1);
% avga = avga*dist(x+1)/x;
% plot(0:length(nll(:,1))-1,avga);
title('Speed vs Distance', 'FontSize', 16);
xlabel('Distance (m)', 'FontSize', 12);
ylabel('Speed (m)', 'FontSize', 12);
hold off;
...
for i=1:x+1
    if dist(i) >= distance_to_travel 
         distance_to_travel = distance_to_travel + distance_per_segment(step);
         plot(track-1:i-1,dist(track:i));
         track = i;
         step = step + 1; 
    end
end
plot(track-1:x,dist(track:x+1));
